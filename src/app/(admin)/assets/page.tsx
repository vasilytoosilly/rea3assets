...
import { PageHeader, Button, Badge, Input, Select, StatusBadge, EmptyState, ErrorBanner, DynamicIcon, Skeleton, SkeletonRow } from "@/components/ui";
...
interface AssetVersionSummary { id: string; version: string; status: string; }
interface AssetSummary {
  id: string; asset_type_id: string; slug: string; name: string; description: string | null;
  division: string; status: string; created_at: string; updated_at: string; published_at: string | null;
  asset_type: AssetTypeSummary; versions: AssetVersionSummary[]; _count: { versions: number; thumbnails: number };
}

const STATUS_OPTIONS = [
  { value: "draft", label: "Draft" }, { value: "in_review", label: "In Review" },
  { value: "approved", label: "Approved" }, { value: "published", label: "Published" },
  { value: "deprecated", label: "Deprecated" }, { value: "archived", label: "Archived" },
];
...
export default function AssetsPage() {
  ...
  const fetchAssets = useCallback(async () => {
    try { setLoading(true); setError(null);
      ...
      setAssets(body.data); setTotalPages(body.pagination.pages); setTotal(body.pagination.total);
    ...
    <div className="space-y-6">
      ...
      {loading && (
        <div className="overflow-hidden rounded-xl border border-[var(--border-default)]">
          <div className="border-b border-[var(--border-default)] bg-[var(--bg-surface)] px-4 py-3">
            <div className="flex gap-4" aria-hidden="true"><Skeleton className="h-4 w-24" /><Skeleton className="h-4 w-16" /><Skeleton className="h-4 w-16" /><Skeleton className="h-4 w-24" /><Skeleton className="h-4 w-20" /></div>
          </div>
          {Array.from({ length: 8 }).map((_, i) => (<SkeletonRow key={i} />))}
        </div>
      )}

      {!loading && error && <ErrorBanner message={error} onRetry={fetchAssets} onDismiss={() => setError(null)} />}

      {!loading && !error && assets.length === 0 && (
        <EmptyState icon={<Package size={48} />} title={statusFilter || search ? "No matching assets" : "No assets yet"}
          description={statusFilter || search ? "Try different filters or search terms." : "Create asset types first, then add assets of each type."}
          action={!statusFilter && !search ? <Button onClick={() => router.push("/assets/new")}><Plus size={16} /> New Asset</Button> : undefined} />
      )}
              ...
              <tbody>
                {assets.map((asset) => (
                  <tr key={asset.id}
                    ...
                    <td className="hidden px-4 py-3 text-right lg:table-cell"><span className="text-xs text-[var(--text-muted)]">{new Date(asset.created_at).toLocaleDateString()}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {!loading && !error && totalPages > 1 && (
        <div className="flex items-center justify-center gap-4">
          ...
          <span className="text-sm text-[var(--text-muted)]">Page <span className="text-[var(--text-primary)]">{page}</span> of <span className="text-[var(--text-primary)]">{totalPages}</span></span>
